...
Expected: every current Android command exits 0. The lifecycle regression delivered by Signal Chronicle Task 10 passes without any Android-plan code edit and the VM suite has no failures. The deleted browser-only test and Web build are historical evidence, not Android release gates. Stop on any warning promoted by `flutter analyze` or any test failure.
